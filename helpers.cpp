...
vector<vector<float>> blur(vector<vector<float>> grid, float blurring)
{
	...
	for (int i = 0; i < grid.size(); i++)
	{
		vector<float> row(grid[0].size(), 0.0);
		newGrid.push_back(row);
	}

	float centerProb = 1.0 - blurring;
	float cornerProb = blurring / 12.0;
	float adjProb = blurring / 6.0;

	vector<float> first_row = {cornerProb, adjProb, cornerProb};
	vector<float> second_row = {adjProb, centerProb, adjProb};
	vector<float> third_row = {cornerProb, adjProb, cornerProb};
	vector<vector<float>> window = {first_row, second_row, third_row};
		...
		for (int j = 0; j < grid[0].size(); j++)
		{
			for (int dx = -1; dx < 2; dx++)
			{
				for (int dy = -1; dy < 2; dy++)
				{
					...
					int newY = (i + dy) % grid.size();
					int newX = (j + dx) % grid[0].size();
					newGrid[newY][newX] += mult * grid[i][j];
				}
			}
		}
	}
